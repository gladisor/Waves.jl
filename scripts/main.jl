...
lr = 5e-6
decay_rate = 1.0f0
MODEL_PATH = mkpath(joinpath(main_path, "models/UnboundedWave/nfreq=$(nfreq)_hsize=$(h_size)_act=$(activation)_gs=$(latent_grid_size)_ele=$(latent_elements)_hor=$(horizon)_in=$(wave_input_layer)_bs=$(batchsize)_pml=$(pml_scale)_lr=$(lr)_decay=$(decay_rate)"))
steps = 20
epochs = 1000
