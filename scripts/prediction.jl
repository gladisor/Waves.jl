...
@time data = [Episode(path = "/home/012761749/AcousticDynamics{TwoDim}_Cloak_Pulse_dt=1.0e-5_steps=100_actions=200_actionspeed=250.0_resolution=(128, 128)/episodes/episode$i.bson") for i in 455:500]
...
for h in horizon
    ...
    @time energy = no_pml_model(s, a, t)[:, 3, :]
    # @time energy_node = node_model(s, a, t)
    y_sc = y[:, 3, :]

    # pml_error = cpu(vec(Flux.mse(y_sc, energy_pml, agg = x -> Flux.mean(x, dims = 1))))
    err = cpu(vec(Flux.mse(y_sc, energy, agg = x -> Flux.mean(x, dims = 1))))
    # error = cpu(vec(Flux.mse(y_sc, energy_node, agg = x -> Flux.mean(x, dims = 1))))
    push!(no_pml_error, err)
    BSON.bson("node_error.bson", horizon = horizon, error = no_pml_error)
end

# val_loader = Flux.DataLoader(EpisodeDataset(data, 200, get_energy_data); data_loader_kwargs...)
val_loader = Flux.DataLoader(prepare_data(data, 200); data_loader_kwargs...)
...
push!(
    no_pml_error, 
    cpu(vec(Flux.mse(y_sc, energy, agg = x -> Flux.mean(x, dims = 1))))
    )

BSON.bson("no_pml_error.bson", horizon = horizon, error = no_pml_error)