...
@everywhere function distributed_calculation_wrapper(batch, gpu_id, models, loss_func)
    model = models[gpu_id]
    Flux.device!(gpu_id)
    display(Flux.device())
    s, a, t, y = gpu(Flux.batch.(batch))
    loss, gs = compute_gradients(model, loss_func, s, a, t, y)
    if first_run
        gs_flat_accumulator, re = Flux.destructure(gs)
        gs_flat_accumulator .*= 0.0f0
        first_run = false
    end
    println("$display(Flux.device())")
    return loss, gs
end
...
function train!(
    ...
    @everywhere train_loss_accumulator = Vector{Float32}()

    @everywhere first_run = true
    @everywhere model = initialize_model()
    # @everywhere _model = cpu(model)
    @everywhere models = [deepcopy(model) for i in 1:nprocs()]
    # M = SharedArray{Float32}()

    for epoch in 1:epochs
        for batches in zip(train_loader_gpu_split...)
            @everywhere global ret_tuple
            
            # batch_1 = Flux.batch.(first(train_loader_gpu_split[1]))
            # batch_2 = Flux.batch.(first(train_loader_gpu_split[2]))
            # batch_3 = Flux.batch.(first(train_loader_gpu_split[3]))

            # data = [batch_1, batch_2, batch_3]

            # ret_A = @spawnat :2 distributed_calculation_wrapper(batch_1, 1, models)
            # ret_B = @spawnat :3 distributed_calculation_wrapper(batch_2, 2, models)
            # # ret_C = @spawnat :4 distributed_calculation_wrapper(batch_3, 3, model)
            
            # wait(ret_A)
            # wait(ret_B)
            # # wait(ret_C)

            # future_arr = [ret_A, ret_B]#, ret_C]
            ...
            @sync @distributed for (i, w) in collect(enumerate(workers()))
                gpu_id = w % 4 # == 3 ? 1 : w % 4
                Flux.device!(gpu_id)
                display(Flux.device())
                println("1")
                ...
                println("2")
                s, a, t, y = gpu(Flux.batch.(batches[i]))
                println("3")
                @time loss, gs = compute_gradients(cur_model, loss_func, s, a, t, y)
                println("6")
                gs_flat, _ = Flux.destructure(gs)
                println("7")
                gs_flat = cpu(gs_flat)

                println("before first_run check")
                if first_run
                    gs_flat_accumulator, re = Flux.destructure(gs)
                    gs_flat_accumulator .*= 0.0f0
                    first_run = false
                println("first_run = false")
                end

                @time global ret_tuple = loss, gs
            end

            # gradients = [@spawnat p distributed_calculation_wrapper(data[p % 3], p % 4, models, loss_func) for p in workers()]
            # gradients = fetch.(gradients)

            for (i, w) in enumerate(workers())
                loss, gs = @fetchfrom w ret_tuple
                # loss, gs = fetch([future_arr[i]])
                gs = gs
...
@time @everywhere env = BSON.load(joinpath(DATA_PATH, "env.bson"))[:env]
...
number_of_gpus = desired_worker_count
...
train_loader_gpu_split = [Flux.DataLoader(prepare_data(train_data_gpu_split[i], horizon); data_loader_kwargs...) for i in 1:length(train_data_gpu_split)]
